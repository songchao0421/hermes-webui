...
### 🔐 Token 认证保护 API
后端默认启用 Bearer Token 认证，Token 自动生成并存储在 `~/.hermes/hermes-webui/auth_token`，启动日志中会打印。可通过 `--no-auth` 参数禁用。

**Token-based API auth.** The backend auto-generates a Bearer token stored at `~/.hermes/hermes-webui/auth_token`. Shown in startup logs. Disable with `--no-auth`.
