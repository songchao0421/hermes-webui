...
<img src="docs/banner.png" alt="Hermes WebUI" width="800">

# Hermes WebUI

**The Dashboard for Hermes — 给你的本地 AI Agent 一张脸**  
...
### 🔒 100% 本地运行，数据不出门
所有数据存在你自己的机器上（`~/.hermes/` 和 `~/.hermes/hermes-webui/`），没有云端服务器，没有账号注册，没有数据上传。断网也能用。

**Runs 100% locally.** All data lives on your machine (`~/.hermes/` and `~/.hermes/hermes-webui/`). No cloud server, no account, no data upload. Works offline.
...
双认证体系：首次访问自动生成 Bearer Token（存储于 `~/.hermes/hermes-webui/auth_token`）；同时支持员工实名账号登录/注册，密码经 bcrypt 哈希存储，支持登出吊销 Token。注册默认开放（内网场景），如需关闭通过 `HERMES_REGISTRATION_ENABLED=false` 环境变量。所有 API 均受限流保护。可通过 `--no-auth` 参数禁用。

**Dual auth: Token-based + user accounts.** Auto-generates a Bearer token at `~/.hermes/hermes-webui/auth_token`. Also supports real-name account registration/login with bcrypt password hashing, logout with token revocation. All APIs rate-limited. Disable with `--no-auth`.
...
```bash
# 1. 克隆项目
git clone https://github.com/songchao0421/hermes-webui.git
cd hermes-webui
...
| Linux | `./scripts/start.sh` | 标准启动，需先 `./scripts/install.sh`（自动创建 .venv） |
...
# 或禁用认证（开发/测试用）
python backend/app.py --no-auth
```

手动启动时请先在项目根目录创建并激活虚拟环境（`venv/`），避免 Debian/Ubuntu 的 PEP 668 冲突。Windows 启动脚本已内置此流程。`launch.bat` / `start-windows.bat` 会自动处理。`start.sh` 依赖 `.venv`（由 `install.sh` 创建）。
...
| 🔐 **Token + 账号双认证** | Bearer Token 自动生成 + 员工实名账号注册/登录，密码 bcrypt 哈希存储；登出自动吊销 Token |
| 🚦 **API 限流** | 全 API 覆盖限流（登录/注册/聊天/记忆/文件上传/登出等），防暴力破解和批量请求过载 |
...
└─────────────────┘    │         │            └────────────────┘  │
                 ...
                 │                 │                  │
     ┌───────────▼──┐   ┌─────────▼────┐   ┌─────────▼───────────┐
     │  ~/.hermes/  │   │  ~/.hermes/hermes-webui/  │   │  Ollama API           │
...
Hermes WebUI 会自动读取 Hermes 的配置，通常不需要手动配置。数据目录默认在 `~/.hermes/hermes-webui/`（旧安装的用户从 `~/.hermes-webui/` 自动迁移）。如需自定义：

```yaml
# ~/.hermes/hermes-webui/config.yaml 或参考 config/hermes-webui.yaml
...
|   ├── ratelimit.py                  # 限流模块（全 API 速率限制）
|   ├── audit.py                      # 结构化审计日志（JSON 格式，记录安全敏感操作）
|   ├── user_auth.py                  # 用户账号管理（注册/登录/登出，bcrypt 密码哈希）
|   ├── _hermes_sdk_bridge.py         # Hermes Agent SDK 桥接层（AIAgent.run_conversation() 封装）
│   ├── hermes_wrapper.bat            # Windows 下调用 WSL Hermes 的包装脚本（备用）
│   ├── requirements.txt
│   ├── routers/                      # API 路由模块（9 个文件）
│   │   ├── agent.py                  #   /api/agent/ — 聊天、文件上传、Agent 执行
│   │   ├── auth.py                   #   /api/auth — 登录、注册、登出、更改密码
│   │   ├── admin.py                  #   /api/admin — 管理操作
│   │   ├── files.py                  #   /api/files — 文件下载、上传
│   │   ├── memories.py               #   /api/memories — 记忆读写
│   │   ├── onboarding.py             #   /api/onboarding — 引导设置
│   │   ├── persona.py                #   /api/persona — 身份配置
│   │   ├── sessions.py               #   /api/sessions — 会话管理
│   │   ├── skills.py                 #   /api/skills — 技能管理
│   │   └── system.py                 #   /api/system — 系统信息、模型切换、自更新
│   └── services/                     # 业务逻辑服务（8 个模块）
│       ├── model_switch.py           #   模型切换逻辑
│       ├── persona_service.py        #   身份读写
│       ├── session_manager.py        #   会话 CRUD + 过期清理（AES-256-GCM 加密存储）
...
│   ├── test_memories.py
│   ├── test_persona.py
│   ├── test_routing.py               # 智能路由测试（关键词评分、纠偏、路由决策）
│   ├── test_security.py
│   └── test_sessions.py
├── config/                           # 配置模板
│   └── hermes-webui.yaml             # WebUI 完整配置模板
├── scripts/                          # 启动/安装脚本
...
项目使用 **pytest** + **FastAPI TestClient**，覆盖认证、记忆、路由、身份、安全、会话六个模块，**55 个测试全部通过**。
...
| 测试文件 | 覆盖内容 |
|---------|---------|
| `test_auth.py` | Token 生成、持久化、Bearer 认证、401 拒绝；用户注册/登录/密码验证 |
| `test_memories.py` | 读写 SOUL/MEMORY/USER.md、非法文件名拒绝、内容长度校验 |
| `test_persona.py` | 获取/更新 Agent 名称、主题预设、自定义颜色、setup_complete |
| `test_security.py` | 头像路径穿越防护、XSS 注入防护、空消息拒绝、超长输入拒绝、认证绕过检测 |
| `test_routing.py` | 智能路由测试（关键词评分、模型选择纠偏、路由决策） |
| `test_sessions.py` | 创建/列出/获取消息/删除会话 |

### 测试隔离

所有测试默认使用独立临时数据目录（`/tmp/hermes-webui-test-*`）运行，`conftest.py` 自动设置 `HERMES_WEBUI_HOME` 环境变量，测试不会污染生产配置文件、Token、Persona 和会话数据。临时目录在测试会话结束后自动清理。
...
### v2.3.4 (2026-05-17) — 安全加固

- **🔒 密码长度升级**：注册/修改密码要求从 6 位提升至 8 位（`MIN_PASSWORD_LENGTH=8`）；`ChangePasswordRequest.old_password` 增加 `max_length=128` 防超长输入
- **🔒 密码历史扩大到 5 次**：`change_password` 和 `admin_reset_password` 均不得重复最近 5 次密码（原为 1 次），`PASSWORD_HISTORY_SIZE=5`
- **🔒 Token 验证反向索引**：`verify_token` 和 `check_token_expired` 改用 dict 反向索引（`token → username`），替代原有 O(n) 逐用户扫描，提升验证安全性
- **🔒 logout 分支一致性**：disabled 用户的登出分支补全 `token_issued_at=0` 清除，与正常登出逻辑保持一致
- **✨ 注册体验优化**：注册成功后自动进入主界面（`location.reload()`），不再切回登录页要求用户二次输入
- **📝 README 格式修复**：修复认证段落残留的 `||` 格式错误

### v2.3.3 (2026-05-17) — 安全修复 + 测试隔离

- **🔒 注册默认开放**：`REGISTRATION_ENABLED` 默认改为 `true`（内网场景），可通过 `HERMES_REGISTRATION_ENABLED=false` 环境变量关闭。不再因未配置管理员账号就无法注册
- **🚦 注册限流放宽**：`/api/auth/register` 限流从 3 次/小时 放宽至 10 次/小时（原值在新用户输错两次密码后就锁到下一小时，导致频繁 429，已修复）
- **💬 429 报错信息修正**：后端 `RateLimitExceeded` handler 返回 `{"error": ...}` 但前端读 `data.detail`，两端字段不一致导致用户只看到"注册失败"看不到真正原因。已统一后端字段为 `detail`，前端也加了 `|| data.error` 双保险 fallback
- **🧪 测试隔离**：`conftest.py` 使用独立临时数据目录（`tmp_path` + `HERMES_WEBUI_HOME` 环境变量），测试不会污染生产配置文件、Token、Persona 和会话数据。临时目录在测试会话结束后自动清理
- **✅ 55/55 测试全部通过**

### v2.3.2 (2026-05-17) — WSL 启动修复 + 测试修复

- **🐛 Bug 修复：`from auth import ...` 导入路径错误**
  - 问题：`python -m backend.app` 运行时 Python 找不到 `backend/` 目录下的 `auth.py`
  - 修复：`app.py` 开头动态将当前目录加入 `sys.path`
- **🐛 Bug 修复：`remove_session_owner` 未定义**
  - 问题：`lifespan` 函数中引用了 `remove_session_owner` 但未从 `session_manager` 导入
  - 修复：在 `app.py` 的 `from services.session_manager import` 中加入该函数
- **🐛 Bug 修复：`_get_user_id()` 不认 auth 开关**
  - 问题：`sessions` 路由的 `_get_user_id()` 始终检查 `request.state.auth_user`，在 `--no-auth` 模式下也返回 401
  - 修复：先检查 `is_auth_enabled()`，auth 关闭时返回默认用户 `"local_user"`
- **🐛 Bug 修复：`test_list_skills` 测试与 API 结构不匹配**
  - 问题：API 返回分类 dict，测试却期望 flat list
- **🛠 WSL 启动脚本修复：`scripts/start-windows.bat`**
  - 行尾混用（CRLF + LF）导致 cmd.exe 解析崩溃
  - 重写为纯 CRLF，增加 `setlocal enabledelayedexpansion`
  - Clean up 重复的 venv/依赖安装段和错乱的步骤编号
- **🐛 Bug 修复：`scripts/start.sh` 缺少颜色变量**
  - `$RED` / `$YELLOW` 引用未定义，增加变量定义
- **✅ 55/55 测试全部通过**
...
- [x] 技能浏览器 + ZIP 导入
- [x] 模型切换器
- [x] 多会话 + 历史持久化（`~/.hermes/hermes-webui/sessions/`）
- [x] 动态主题引擎（5 预设 + 自定义）
- [ ] 中英双语界面（自动检测浏览器语言）
- [x] 流式响应（SSE）
